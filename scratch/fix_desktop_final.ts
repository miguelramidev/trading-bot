...
content = content.replace(
    /Text\('\+\\\$45,210\.80 \(\+3\.77\%\) PnL ACTIVO'[^)]*\)/,
...
// 2. Fix Liquidez / Margen Escaping
content = content.replace(
    /Text\('\\\$\{\_freeBalance\.toStringAsFixed\(2\)\} USDT/g,
    `Text('\\$\\$\\{_freeBalance.toStringAsFixed(2)\\} USDT` // wait, easier to just do simple replace:
);
content = content.replace(
...
// 4. Fix Hardcoded Trades list
const hardcodedTrades = `          _tradeRow('BTC/USDT', 'LONG 5x', '\\$45,000.00', '+\\$1,840.20', '+4.12%'),
          const Divider(color: AppColors.border, height: 32),
          _tradeRow('ETH/USDT', 'LONG 5x', '\\$25,000.00', '+\\$620.80', '+2.45%'),
          const Divider(color: AppColors.border, height: 32),
          _tradeRow('SOL/USDT', 'SHORT 2x', '\\$18,500.00', '+\\$312.40', '+1.68%'),`;

const dynamicTrades = `          if (_isLoading)
...
content = content.replace(hardcodedTrades, dynamicTrades);
