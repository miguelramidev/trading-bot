...
- [x] **[COMPLETADO] Expansión Completa a TODOS los Símbolos Operables de Exness Cent (`USDc`):** Verificado en vivo en terminal MT5 e incorporados en `bot.py` literalmente **los 32 símbolos reales operables** de la cuenta Exness Cent (`USDc` / `USCc`).
  - **Símbolos incorporados (32 en total):** Metales (`XAUUSDc`, `XAGUSDc`), Cripto (`BTCUSDc`, `BTCUSDTc`, `ETHUSDc`, `ETHBTCc`), USD Majors (`EURUSDc`, `GBPUSDc`, `AUDUSDc`, `NZDUSDc`, `USDJPYc`, `USDCHFc`, `USDCADc`, `USDHKDc`), Cruces EUR (`EURAUDc`, `EURCADc`, `EURCHFc`, `EURGBPc`, `EURJPYc`, `EURNZDc`), Cruces GBP (`GBPAUDc`, `GBPCADc`, `GBPCHFc`, `GBPJPYc`, `GBPNZDc`), Cruces JPY (`AUDJPYc`, `CADJPYc`, `CHFJPYc`, `NZDJPYc`) y Cruces de Materias Primas (`AUDCADc`, `AUDCHFc`, `AUDNZDc`).
  - **Regla de Correlación:** Organizados en 7 grupos (`METALS`, `CRYPTO`, `USD_MAJORS`, `EUR_CROSSES`, `GBP_CROSSES`, `JPY_PAIRS`, `COMMODITY_CROSSES`) impidiendo duplicidad direccional y respetando el límite global estricto de **3 operaciones tradicionales concurrentes**.
