    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        toastification.show(
          ...
          backgroundColor: AppColors.winGreen.withOpacity(0.1),
...
content = content.replace(
    ...
    "toastification.show(context: context, type: ToastificationType.info, style: ToastificationStyle.flatColored, title: const Text('Guardando configuración...'), alignment: Alignment.topCenter, autoCloseDuration: const Duration(seconds: 2));"
);
...
content = content.replace(
    ...
    "toastification.show(context: context, type: ToastificationType.success, style: ToastificationStyle.flatColored, title: const Text('Configuración guardada'), alignment: Alignment.topCenter, autoCloseDuration: const Duration(seconds: 3));"
);

content = content.replace(
    "ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e'), backgroundColor: AppColors.lossRed));",
    "toastification.show(context: context, type: ToastificationType.error, style: ToastificationStyle.flatColored, title: const Text('Error al guardar'), description: Text(e.toString()), alignment: Alignment.topCenter, autoCloseDuration: const Duration(seconds: 4));"
);
