...
- [x] **[COMPLETADO] Expansión Completa del Universo Exness Cent (`USDc`):** Verificado en vivo en terminal MT5 e incorporados en `bot.py` los 20 símbolos reales disponibles para la cuenta Exness Cent (`USDc` / `USCc`).
  - **Símbolos incorporados:** Metales (`XAUUSDc`, `XAGUSDc`), las 7 Divisas Mayores (`EURUSDc`, `GBPUSDc`, `AUDUSDc`, `NZDUSDc`, `USDJPYc`, `USDCHFc`, `USDCADc`), Cruces del Yen (`EURJPYc`, `GBPJPYc`, `AUDJPYc`, `CADJPYc`), Cruces Europeos/GBP (`GBPCHFc`, `GBPCADc`, `EURCHFc`, `EURGBPc`, `EURCADc`) y Cripto Cent (`BTCUSDc`, `ETHUSDc` — *nota: LTCUSDc y BCHUSDc no son soportados por Exness Cent*).
  - **Regla de Correlación:** Organizados obligatoriamente en `self.correlation_groups` (`METALS`, `CRYPTO`, `USD_MAJORS`, `JPY_PAIRS`, `GBP_CROSSES`, `AUD_NZD_CROSSES`, `EUR_CROSSES`) impidiendo duplicidad direccional en el portafolio y respetando el límite global estricto de **3 operaciones tradicionales concurrentes**.
