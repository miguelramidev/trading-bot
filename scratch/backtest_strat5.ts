...
async function run() {
    ...
    try {
        ...
        for (let i = 250; i < candles.length - 1; i++) {
           ...
           if (signalDirection) {
              ...
              let sl = signalDirection === "LONG" ? entry - (1.0 * currentAtr) : entry + (1.0 * currentAtr);
              let tp = signalDirection === "LONG" ? entry + (2.0 * currentAtr) : entry - (2.0 * currentAtr);
  ...
  console.log(`\n=== ESTRATEGIA 5 (MOMENTUM RIDER EMA 9) ===`);
