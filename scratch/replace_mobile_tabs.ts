...
const oldTabs = `Row(
                children: ['1D', '1S', '1M', 'YTD'].map((e) => Container(
                  ...
                  decoration: BoxDecoration(
                    color: e == '1D' ? AppColors.border : Colors.transparent,
                    borderRadius: BorderRadius.circular(6),
                  ),
                  child: Text(e, style: AppTheme.monoStyle.copyWith(color: e == '1D' ? AppColors.winGreen : AppColors.textSecondary, fontSize: 12)),
                )).toList(),
              )`;

const newButton = `InkWell(
                onTap: () {
                  setState(() {
                    _isLoading = true;
                  });
